...
right_addition_params = gen_params(
    [3, 6, 3, 6, 6, [4, 5, 6, 1, 2, 3], 10, [4, 5, 6, 1, 2, 3], ]
)
...
@pytest.mark.parametrize('right, raises', [
...
@pytest.mark.parametrize('value, raises, result', [
    (1, does_not_raise(), 1),
...
def test_transform_other_to_int(value, raises, result) -> None:
    with raises:
        assert RollResult._RollResult__transform_other_to_int(
