...
def test_to_dict(dummy_fb_post: FacebookPost, dummy_post: dict) -> None:
    dummy_post.update({"is_event": False})
    assert dummy_fb_post.to_dict() == dummy_post


@pytest.mark.asyncio()
async def test_save(dummy_fb_post: FacebookPost, client: AsyncMongoMockClient) -> None:
...
@pytest.mark.asyncio()
async def test_latest_timestamp(faker: Faker, post_factory: TPostFactory, client: AsyncMongoMockClient) -> None:
    ...
    assert await FacebookPost.latest_timestamp(client.robomania) == latest_timestamp
...
@pytest.mark.asyncio()
async def test_get_only_new_posts(mocker: MockerFixture, faker: Faker, raw_post_factory: TRawPostFactory) -> None:
    ...
    newest_posts = await FacebookPost.get_only_new_posts(None, list(map(FacebookPost.from_raw, posts)))

    assert all(i["timestamp"] == j.timestamp for i, j in zip(map(vars, sorted_posts[6:]), newest_posts))
...
def test_event_posts(
    ...
    assert all(i.timestamp == j["timestamp"] for i, j in zip(FacebookPost.remove_event_posts(pr), [p0, p2, p3]))
